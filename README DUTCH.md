# Next_heatpump — Home Assistant-integratie
...
Een HACS-compatibele custom integratie voor de **Heative Next R290 warmtepomp**, die communiceert via **Modbus TCP** over een RS485-naar-WiFi-gateway.

Geen YAML nodig. Alle instellingen worden via de Home Assistant-UI gedaan. Voor het dashboard en het tijdschema zijn echter een extra repository en YAML-bestanden nodig. Zie installatie via HACS: https://github.com/nielsfaber/scheduler-component  

### Deze software is een fork van de Adlar Aurora II-repository, ontwikkeld door https://github.com/conradhagemans.  
---

## Hardware-opzet

Deze integratie is ontwikkeld en getest met de volgende hardware:   
...
- **E810-R21 Ebyte** RS485-splitter (2 master / 1 slave): 
  - **Slave-poort** → Modbus RS485-kabel vanaf de warmtepomp
  - **Master 1** → Elfin EW11 4G-module
  - **Master 2** → Elfin EW11A (RS485-naar-WiFi/TCP-bridge)
- De **Elfin EW11A** geconfigureerd als TCP-server op poort **502**
- Modbus slave-ID van de warmtepomp: **1** (standaard)

[E810-Rx1+User+Manual_EN_v1.0.pdf](https://github.com/user-attachments/files/31154340/E810-Rx1%2BUser%2BManual_EN_v1.0.pdf)

Elke RS485-naar-Modbus-TCP-bridge zou moeten werken.

### Of koppel het TFT-display los en gebruik deze repository in plaats daarvan met de EW11A. Beide tegelijk aangesloten op de rs485-displayaansluiting WERKT NIET!
---

## Elfin EW11-configuratie
...
2. Klik op het menu met de drie puntjes → **Aangepaste repositories**
3. Voeg deze repository-URL toe, categorie: **Integratie**
4. Zoek naar **Next Heatpump** en installeer
5. Herstart Home Assistant

### Handmatig

Kopieer de map `custom_components/next_heatpump/` naar de map `config/custom_components/` van je HA-installatie en herstart daarna.
...
2. Zoek naar **Next Heatpump**
3. Vul in:
   - **IP-adres** van je RS485-TCP-gateway
   - **Poort** (standaard `502`)
   - **Slave-ID** (standaard `1`)
   - **Scaninterval** in seconden (standaard `45`)

---
## Schema normaal/eco-modus
...
### Diverse binaire sensoren en storingssensoren
...
## "Geforceerde besturing (compressor/ventilator)"
...
### Scaninterval
Het standaard scaninterval is 45 seconden. Met ongeveer 40 registers × 200ms vertraging = ongeveer 8 seconden per pollcyclus, wordt een minimum van 30 seconden aanbevolen.

---

### VRIJWARING
**Deze integratie is community-ontwikkeld en niet gelieerd aan Heative, Adlår of SolarEast. GEBRUIK OP EIGEN RISICO. Onjuiste schrijfacties naar besturingsregisters kunnen de werking van de warmtepomp beïnvloeden. Controleer setpoints altijd voordat je wijzigingen doorvoert.** Gebruik van het logo is goedgekeurd door Heative, bedankt!
